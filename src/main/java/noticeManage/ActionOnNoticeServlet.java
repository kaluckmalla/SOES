 ...
 */
@WebServlet("/ActionOnNoticeServlet")
@MultipartConfig(
...
public class ActionOnNoticeServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		int ActionNoticeId=Integer.parseInt(request.getParameter("snoticeId"));//here all i.e edit delete and update process is done from same id by giving same name value for id
		String fileNameFromDB=request.getParameter("snFileNameFromDB");
		...
		try {
			...
			if(action.equals("Edit")) {//edit button chahi another page i.e. update page maa janako laagi use gareko x by passing value from db with the help of id
				...
				ps.setInt(1,ActionNoticeId);
				rs=ps.executeQuery();
				while(rs.next()) {//can be used if statement for only one data slection from resultset both work same


					request.setAttribute("editId",ActionNoticeId );
					request.setAttribute("editDate", rs.getString(2));
					request.setAttribute("editTitle", rs.getString(3));
					request.setAttribute("editFileName",rs.getString(4));



				}
				RequestDispatcher dispatcher=request.getRequestDispatcher("/AdminManagePages//updatenotice.jsp");
			...
			//if click on delete button
				...
				String path=UPLOAD_DIRECTORY + File.separator + fileNameFromDB; //file path from server to check if file exist or not
				...
				if(directory.exists()) {//if file exist then delete it from file server & database
					...
					request.setAttribute("snResult", "Post "+ ActionNoticeId+" was deleted successfully.");
					RequestDispatcher dispatcher=request.getRequestDispatcher("/AdminManagePages//actiononpostnotice.jsp");
					dispatcher.forward(request, response);

				}
				else{//if file no exist in file server
					request.setAttribute("snResult","Post "+ActionNoticeId+" failed to delete due to file not exist in server.");
					RequestDispatcher dispatcher=request.getRequestDispatcher("/AdminManagePages//actiononpostnotice.jsp");
					dispatcher.forward(request, response);
				}
				ps.close();//tala aruko laagi use garne hunaale leak nahos bhanera close gareko
			...
			//if click on update input element (which is button) in update page for update it uses same notice as update/delete
				...
				while(rs.next()) {//if use garepani hunx for only one row in resultset while work same as if
					if(!edittitle.equals(rs.getString(3))){
						ps1=con.prepareStatement("update notice set title='"+edittitle+"' where id='"+ActionNoticeId+"'");
