...
<title>Action on Civil Material</title>
...
<body>
	...
	<form method="post" action="/SOES/SearchCivilStudyMaterialIdServlet">

		<div class="container-fluid d-flex justify-content-center"
			style="background-color: #ECECEC;">
			<div class="row  col-sm-10 d-flex justify-content-center ">
				<h2 class="text-center text-white bg-danger">Search Civil Material By
					ID</h2>


				<input class="col-7 col-sm-4" type="number" name="searchcivilstudymaterialid"
					placeholder="type civil material id here" required>
	...
	<form method="post" action="/SOES/ActionOnCivilStudyMaterial"
		enctype="multipart/form-data">

		<div class="container-fluid d-flex justify-content-center"
			style="background-color: #ECECEC;">
			<div class="row col-12  col-sm-10">
				<h4 class="text-danger fsw-italic text-center text-break mt-2">${sResult}</h4>

				<div class=" mb-2 col-6 col-sm-6" id="hide1">
					<h5 class="text-primary">${sId}</h5>
					<input class="h6" type="number" name="searchId" value="${Id}"
						id="inputfieldForId" required readonly style="width: 40%;" />
				</div>
				<div class="mb-2 col-6 col-sm-6">
					<h5 class="text-primary">${sDate}</h5>
					<h6>${Date}</h6>

				</div>
				<div class="mb-2 col-6 col-sm-6">
					<h5 class="text-primary">${sFaculty}</h5>
					<h6>${Faculty}</h6>

				</div>
				<div class="mb-2 col-6 col-sm-6">
					<h5 class="text-primary">${sSemester}</h5>
					<h6>${Semester}</h6>

				</div>
				<div class="mb-2 col-6 col-sm-6">
					<h5 class="text-primary">${sType}</h5>
					<h6>${Type}</h6>

				</div>
				<div class="mb-2 col-6 col-sm-6">
					<h5 class="text-primary">${sSubjectName}</h5>
					<h6>${SubjectName}</h6>

				</div>
				<div class="mb-2 col-12 col-sm-12">
					<h5 class="text-primary">${sFileName}</h5>
					<h6>${FileName}</h6>

				</div>
				
				
				<h5 class="col-12 col-sm-12 text-danger">${sAction}</h5>

				
				<div class="col-12 col-sm-12" id="hide2">
					<input class="h5" type="submit" name="action"
	...
	<div class="container-fluid d-flex justify-content-center"
		style="background-color: #ECECEC;">
			...
			<h2 class="text-center text-white bg-danger mt-3">Civil
				Engineering Material List</h2>

			<table class="table-bordered text-center  text-break">
				<tr class="text-danger bg-info">
					<th class="h5">Id</th>
					<th class="h5">Date</th>								
					<th class="h5">Semester</th>
					<th class="h5">Type</th>
					<th class="h5">Subject Name</th>
				...
				try {
					...
					for (AddPostStudyMaterialClass list : alllist) {
						if(list.getpFaculty().equals("Civil Engineering")){
				%>
				<tr style="background-color: #F5F5DC; height: 3rem;">

					<td class="h6 fw-normal" width="50rem"><%=list.getpId()%></td>
					<td class="h6 fw-normal" width="50rem"><%=list.getpDate()%></td>				
					<td class="h6 fw-normal" ><%=list.getpSemester()%></td>
					<td class="h6 fw-normal" ><%=list.getpType()%></td>
					<td class="h6 fw-normal" ><%=list.getpSubject()%></td>
					<td class="h6 fw-normal" ><%=list.getpFile()%></td>
